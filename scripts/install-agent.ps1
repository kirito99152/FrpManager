# FrpManager Agent - Windows Install Script
...
echo "Downloading Agent for Windows from FrpManager Server..."
...
$oldService = Get-Service -Name "FrpManagerAgent" -ErrorAction SilentlyContinue
if ($oldService) {
    echo "Removing existing service..."
    Stop-Service -Name "FrpManagerAgent" -Force -ErrorAction SilentlyContinue
    sc.exe delete "FrpManagerAgent"
    Start-Sleep -s 2
}

# Create new service
New-Service -Name "FrpManagerAgent" `
            -BinaryPathName "`"$InstallDir\agent.exe`" -server $Server" `
            -DisplayName "FrpManager Client Agent" `
            -StartupType Automatic `
            -Description "Monitors host and manages FRP tunnels"

echo "Starting service..."
Start-Service -Name "FrpManagerAgent"
